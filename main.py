...
# --- 💾 ระบบบันทึกข้อมูลผู้เล่น (รองรับระบบ Bank & Fix Sync) ---
...
def save_player_data():
    try:
        ...
        # ✅ แก้ไขให้บันทึกฟิลด์ 'bank' และปรับชื่อคีย์ให้ตรงกับ Roblox Script
        ref = db.reference(f'UsersID/{user_id}')
        ref.update({
            'InGameName': data.get('username'),
            'money': data.get('money'),      # ใช้ตัวเล็กตามสคริปต์ส่งมาเพื่อความสัมพันธ์ของข้อมูล
            'bank': data.get('bank'),        # 🟢 เพิ่มบรรทัดนี้เพื่อให้ยอดเงินธนาคารบันทึกลง Firebase
